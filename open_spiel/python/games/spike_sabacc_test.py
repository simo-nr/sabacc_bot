...
from open_spiel.python.games import spike_sabacc
import pyspiel


class SpikeSabaccTest(absltest.TestCase):

  def test_can_create_game_and_state(self):
    """Checks we can create the game and a state."""
    game = spike_sabacc.SpikeSabacc({"hand_length": 3, "num_digits": 3})
  ...
  def test_basic_bid(self):
    ...
    bid_offset = spike_sabacc.BID_ACTION_OFFSET
    ...
    for next_action in state.legal_actions():
      if next_action == spike_sabacc.CHALLENGE_ACTION:
        continue
  ...
  def test_single_random_round(self):
    ...
    challenge = spike_sabacc.CHALLENGE_ACTION
  ...
  def test_single_deterministic_round(self):
    ...
    state.apply_action(state.encode_bid(4, 1) + spike_sabacc.BID_ACTION_OFFSET)
    # Player 1 challenges.
    state.apply_action(spike_sabacc.CHALLENGE_ACTION)
  ...
  def test_game_from_cc(self):
    ...
    game = pyspiel.load_game("spike_sabacc", {"players": 2})
  ...
  def test_pickle(self):
    ...
    game = pyspiel.load_game("spike_sabacc")
